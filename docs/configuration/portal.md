...
account numbers, transaction details, or credentials. A stored run without a
timestamp or a bank list, such as one a hand edit left, is left out of the list,
and so is a bank row that is not an object; the file itself is not changed.
