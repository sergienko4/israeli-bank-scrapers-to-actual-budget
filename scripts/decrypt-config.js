 ...
 * Usage: CONFIG_PASSWORD=mypassword node scripts/decrypt-config.js [input]
 */
...
if (!password) {
  ...
  console.error('   Example: CONFIG_PASSWORD=mypassword node scripts/decrypt-config.js');
  process.exit(1);
}
...
try {
  ...
  console.log('⚠️  Re-encrypt after editing: CONFIG_PASSWORD=... node scripts/encrypt-config.js');
} catch (error) {
