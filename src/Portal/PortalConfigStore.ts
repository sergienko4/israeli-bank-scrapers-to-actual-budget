...
import { coerceTargetAccounts, hashPlainPortalPassword, maskSecrets, pruneEmptyNotificationChannels, pruneEmptyOptionalSections, restoreMasked } from './ConfigMutations.js';
...
/**
 * Applies the portal's save-time shaping used for validation: coerces target
 * accounts, then strips the empty navigation artifacts the UI materializes
 * (empty notification channels first, then empty top-level optional sections),
 * so the "Validate" preview and the save gate judge the very same shaped config.
 * Password hashing is deliberately excluded — it is a save-only concern.
 * @param config - Candidate config with round-tripped masked secrets restored.
 * @returns The shaped config the gate/preview should judge.
 */
function shapeForValidation(config: IImporterConfig): IImporterConfig {
  const coerced = coerceTargetAccounts(config);
  const channelsPruned = pruneEmptyNotificationChannels(coerced);
  return pruneEmptyOptionalSections(channelsPruned);
}

/**
 * Shapes a candidate config — restores round-tripped masked secrets, hashes a
 * freshly-typed portal password, and applies {@link shapeForValidation} — then
 * gates it on the importer's exact boot rules plus the portal's own auth
 * bootability (see {@link collectGateErrors}). Pure transform + validation, no I/O.
...
export default class PortalConfigStore {
   ...
   * round-tripped masked secrets restored and the same shaping the save path
   * applies (`shapeForValidation` — target coercion plus empty-artifact
   * pruning), so the live "Validate" preview judges the real secret values and
   * the exact shaped config a save would persist (e.g. a masked Telegram
   * botToken is not reported as a format error it does not actually have, and an
   * empty notification channel is not flagged as a missing URL). This is an
   * offline health report, not the full save gate: it does not run the
   * boot-credential or portal-auth checks `collectGateErrors` adds, so a
   * green preview is necessary but not sufficient for a successful save.
  ...
  public validate(next: IImporterConfig): IValidationResult[] {
    ...
    const shaped = shapeForValidation(restored);
    return ConfigValidator.validateOffline(shaped);
  }
