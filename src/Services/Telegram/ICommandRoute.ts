...
/** Per-dispatch context passed to a route handler. */
export interface ICommandCtx<TDeps> {
  /** The full original command/callback string after trim. */
  readonly raw: string;
  /** The first whitespace-trimmed token (lowercased for slash commands). */
  readonly command: string;
  /** Per-handler dependencies. Each route closes over its own subset. */
  readonly deps: TDeps;
}
 ...
 */
export interface ICommandRoute<TDeps> {
