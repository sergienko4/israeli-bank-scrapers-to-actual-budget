...
/**
 * Closes a store descriptor best-effort, swallowing any failure.
 *
 * <p>A close that fails after the read succeeded must not discard what was
 * read. It would surface as an empty store, and the write path treats an
 * unreadable store as damaged: a healthy file would be quarantined and
 * replaced with only the account being written, stranding every sibling
 * token in the quarantine copy at the cost of an SMS each.
 * @param descriptor - Open descriptor to close.
 * @returns True when the descriptor closed, false when the error was swallowed.
 */
function closeStoreFile(descriptor: number): boolean {
  try {
    closeSync(descriptor);
    return true;
  } catch {
    // Best-effort: the descriptor is abandoned either way, and the value the
    // operation already produced is worth more than the close error.
    return false;
  }
}
