...
describe('bootPortal', () => {
  ...
  it('starts and returns true when enabled', async () => {
    loadRaw.mockReturnValue(succeed(fakeImporterConfig({ portal: { enabled: true, port: 0, sessionSecret: 'a-strong-session-secret', passwordHash: PASSWORD_HASH } })));
  ...
  it('returns false when the auth config is incomplete for the mode', async () => {
    loadRaw.mockReturnValue(succeed(fakeImporterConfig({
      portal: { enabled: true, port: 0, sessionSecret: 'a-strong-session-secret', authMode: 'both' },
    })));
    expect(await bootPortal()).toBe(false);
    expect(startPortal).not.toHaveBeenCalled();
  });
