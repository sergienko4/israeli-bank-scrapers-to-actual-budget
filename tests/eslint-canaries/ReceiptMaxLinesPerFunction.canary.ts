// Canary: PR 14 — must trigger `max-lines-per-function: 10`
// for src/Services/Receipt/**.
//
// The chore(eslint) commit on PR 14 adds Section 7k to
// eslint.config.mjs that tightens `max-lines-per-function` from the
// default 20 down to 10 for:
//   - src/Services/Receipt/**/*.ts
//   - tests/eslint-canaries/ReceiptMaxLinesPerFunction.canary.ts ← this file
//
...
//
// Any new file landing under src/Services/Receipt/** MUST keep every
...
export default function oversizedReceiptSample(): number {
  const a = 1;
  const b = 2;
  const c = 3;
  const d = 4;
