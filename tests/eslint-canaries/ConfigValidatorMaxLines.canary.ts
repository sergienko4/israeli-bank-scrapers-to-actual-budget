// Canary: should trigger `max-lines: 200` cap on ConfigValidator.ts.
//
// The PR 3 rule (Section 7c of eslint.config.mjs) tightens `max-lines`
// from the default 300 down to 200 for:
//   - src/Config/ConfigValidator.ts
//   - tests/eslint-canaries/ConfigValidatorMaxLines.canary.ts  ← this file
//
// After PR 3, ConfigValidator.ts is a thin orchestrator (~105 LoC) that
// delegates to per-block modules under `src/Config/Validators/`. Adding
// a new offline / online check MUST land in a new (or existing) checker
// module — never inline in ConfigValidator.ts. This canary exceeds 200
// effective LoC so the canary harness can prove the rule is alive on
// every commit.
//
...
// nullable returns, no string concatenation patterns the architectural
// rules ban). It is NOT exercised at runtime.
