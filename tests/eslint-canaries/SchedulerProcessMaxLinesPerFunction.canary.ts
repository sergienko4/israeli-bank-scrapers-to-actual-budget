// Canary: PR 20b — must trigger `max-lines-per-function: 10`
// for src/Scheduler/Process/**.
//
// The chore(eslint) commit on PR 20b adds Section 7j to
// eslint.config.mjs that tightens `max-lines-per-function` from the
// default 20 down to 10 for:
//   - src/Scheduler/Process/**/*.ts
//   - tests/eslint-canaries/SchedulerProcessMaxLinesPerFunction.canary.ts ← this file
//
...
//
// Any new file landing under src/Scheduler/Process/** MUST keep every
...
export default function oversizedProcessSample(): number {
  const a = 1;
  const b = 2;
  const c = 3;
  const d = 4;
