// Canary: should trigger `max-lines: 80` cap on NotificationService.ts.
//
// The PR 4 rule (Section 7d of eslint.config.mjs) tightens `max-lines`
// from the default 300 down to 80 for:
//   - src/Services/NotificationService.ts
//   - tests/eslint-canaries/NotificationServiceMaxLines.canary.ts  ← this file
//
// After PR 4, NotificationService.ts is a thin orchestrator (~60 LoC) that
// delegates to `Notifications/NotificationGate.ts` (build notifier list) and
// `Notifications/NotificationDispatcher.ts` (parallel dispatch + count).
// Adding a new send action MUST land in the orchestrator only as a 5-line
// delegation to dispatchToAll(...). Anything heavier (rate-limit, dry-run
// gate, retry policy) MUST become a new module under
// `src/Services/Notifications/*`.
//
// This canary exceeds 80 effective LoC so the canary harness can prove the
// rule is alive on every commit.
//
// The fixture is intentionally a flat list of trivial constant declarations
// to keep other ESLint rules quiet (no nested calls, no nullable returns).
