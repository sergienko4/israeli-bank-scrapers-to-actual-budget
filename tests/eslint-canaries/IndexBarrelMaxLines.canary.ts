// Canary: should trigger `max-lines: 50` cap on src/Index.ts.
//
// PR 21 (Section 7h of eslint.config.mjs) tightens `max-lines` from the
// default 300 down to 50 for:
//   - src/Index.ts
//   - tests/eslint-canaries/IndexBarrelMaxLines.canary.ts  ← this file
//
// After PR 21, src/Index.ts is a ~13-LoC barrel that re-exports the
// public Importer surface from src/Importer/* and calls bootImporter()
// when executed directly as a Node entry point. The actual composition
// root lives in src/Importer/ImporterBootstrap.ts. Any new boot-time
// orchestration MUST land in a new module under src/Importer/*, never
// inline in the barrel.
//
// This canary exceeds 50 effective LoC (skipBlankLines + skipComments
// configured) so the canary harness can prove the rule is alive on
// every commit.
//
