// Canary: should trigger max-lines: 200 cap on TelegramPoller.ts.
//
// The PR 7 rule (Section 7f of eslint.config.mjs) tightens `max-lines`
// from the default 300 down to 200 for:
//   - src/Services/TelegramPoller.ts
//   - tests/eslint-canaries/TelegramPollerMaxLines.canary.ts  <- this file
//
// After PR 7, TelegramPoller.ts is a thin lifecycle + error-recovery
// orchestrator (~160 effective LoC) that wires three peers:
// TelegramPollHttp (long-poll HTTP), TelegramPollBackoff (pure backoff
// math), and TelegramUpdateDispatcher (chat-id filtering + text/photo/
// callback routing). Adding a new update kind MUST go to the dispatcher;
// adding a new HTTP endpoint MUST go to TelegramPollHttp; adding a new
// backoff curve MUST go to TelegramPollBackoff.
//
