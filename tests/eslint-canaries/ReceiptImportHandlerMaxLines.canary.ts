// Canary: should trigger max-lines: 200 cap on ReceiptImportHandler.ts +
// Receipt/ReceiptImportFlow.ts.
//
// Section 7v of eslint.config.mjs tightens `max-lines` from the default 300
// down to 200 for:
//   - src/Services/ReceiptImportHandler.ts (thin conversation adapter)
//   - src/Services/Receipt/ReceiptImportFlow.ts (the carved-out engine)
//   - tests/eslint-canaries/ReceiptImportHandlerMaxLines.canary.ts  <- this file
//
// The PR 27 split removed the prior `max-lines: 310` grandfather LOOSENING
// (which had no canary) and replaced it with this tightened, canary-backed
// cap so neither the handler nor the flow can drift back into a god-class.
//
// This canary exceeds 200 effective LoC so the canary harness can prove the
// rule is alive on every commit. It is a flat list of trivial constant
// declarations to keep other ESLint rules quiet, and is NOT run at runtime.
