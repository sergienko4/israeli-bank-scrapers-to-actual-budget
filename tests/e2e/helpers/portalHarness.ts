 ...
 * below Vitest's hook timeout bounds that wait, so a page left navigating can
 * hang teardown outright. Navigating to `about:blank` first cancels the pending
 * load and leaves a document with nothing to object to.
 *
 ...
 *
 * Playwright exposes no timeout on any `close()` — the option bag accepts only
 * `reason` — and internally passes a sentinel meaning "wait forever". Racing a
 * timer cannot cancel that wait, so this does not rescue a wedged resource. What
 * it does is stop one stuck close from consuming the whole hook budget and
 * report *which* resource stalled, instead of an anonymous hook timeout that
